...
### 4.1 SSH: не потерять доступ

Перед запуском деплоя лучше сразу добавить SSH-ключ на сервер и проверить вход по ключу в отдельной сессии.
...
Вставь в `authorized_keys` свой публичный ключ (`ssh-ed25519 ...`).

Проверка в новой локальной сессии:
...
> ⚠️ Не отключай парольный вход, пока не проверил, что вход по ключу реально работает.
...
- Если парольный вход отключён, сначала настрой и проверь вход по SSH-ключу
- Безопасный вариант для root: `PermitRootLogin prohibit-password` — root по ключу разрешён, по паролю запрещён
